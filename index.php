...
if(!empty($_GET['pb']) == $postback_key){
	if(!empty($_GET['cid']) && stristr($_GET['cid'], $cid_delimiter)){
		$get_cid = $_GET['cid'];
	}
	else{
		err_404();
...
if(!empty($_GET['api'])){
	...
	$api_key_host = trim($api_data[0]);
	$id = trim($api_data[1]);
	$ipuser = trim($api_data[2]);
	$referer = trim($api_data[3]);
	$useragent = trim($api_data[4]);
	$se = trim($api_data[5]);
	$domain = trim($api_data[6]);
	$lang = trim($api_data[7]);
	$uniq = trim($api_data[8]);
	$key = trim($api_data[9]);
	$cf_country = trim($api_data[10]);
	$parameter_1 = urldecode(trim($api_data[11]));
	$parameter_2 = urldecode(trim($api_data[12]));
	$parameter_3 = urldecode(trim($api_data[13]));
	$parameter_4 = urldecode(trim($api_data[14]));
	$parameter_5 = urldecode(trim($api_data[15]));
	$counter = $empty;
...
if(empty($_GET['api'])){
	...
	if(preg_match("~^$path\/(.+?)\?.+?=(.+?)?$~", $in_url, $matches)){
		...
		if(empty($parameters)){
			if(!empty($par[0])){$parameter_1 = urldecode(trim($par[0]['value']));}
			if(!empty($par[1])){$parameter_2 = urldecode(trim($par[1]['value']));}
			if(!empty($par[2])){$parameter_3 = urldecode(trim($par[2]['value']));}
			if(!empty($par[3])){$parameter_4 = urldecode(trim($par[3]['value']));}
			if(!empty($par[4])){$parameter_5 = urldecode(trim($par[4]['value']));}
		}
		if(!empty($parameters)){
...
if(!empty($id) && file_exists($ini_folder.'/'.$id.'.ini')){
...
function logs(){
	global $id, $g_name, $s_name, $log_folder, $out, $key, $redirect, $device, $operator, $country, $city, $region, $lang, $uniq, $bot, $ipuser, $referer, $useragent, $domain, $se, $log_period, $db, $empty, $log_bots, $log_out, $log_ref, $log_key, $log_ua, $counter, $timeout_1, $postback, $cid, $cid_length, $index;
	...
	if(!$db->querySingle("SELECT * FROM sqlite_master WHERE type = 'table' AND name = '$table';")){
		...
		$del = strtotime(date("d-m-Y", strtotime('-'.$log_period.' day')));
